-- Combined customer insights views with renamed views

-- 1. EMI Summary
CREATE OR REPLACE VIEW insights_emi_summary AS
WITH date_range AS (
    SELECT customer_id,
...
-- 2. SIP Summary
CREATE OR REPLACE VIEW insights_sip_summary AS
WITH date_range AS (
    SELECT customer_id,
...
-- 3. Salary Summary
CREATE OR REPLACE VIEW insights_salary_summary AS
WITH date_range AS (
    SELECT customer_id,
...
-- 4. Insurance Summary
CREATE OR REPLACE VIEW insights_insurance_summary AS
WITH date_range AS (
    SELECT customer_id,
           MIN(date_trunc('month', transaction_date)) AS first_month,
           MAX(date_trunc('month', transaction_date)) AS last_month,
...
CREATE OR REPLACE VIEW insights_transactions_summary AS
WITH date_range AS (
    SELECT customer_id,
           MIN(date_trunc('month', transaction_date)) AS first_month,
           MAX(date_trunc('month', transaction_date)) AS last_month,
...
-- 6. Loan Summary
CREATE OR REPLACE VIEW insights_loan_summary AS
SELECT
    customer_id,
    cu.name AS customer_name,
...
CREATE OR REPLACE VIEW insights_financial_dashboard AS
SELECT
    c.id AS customer_id,
...
FROM customers c
LEFT JOIN insights_emi_summary emi ON c.id = emi.customer_id
LEFT JOIN insights_sip_summary sip ON c.id = sip.customer_id
LEFT JOIN insights_salary_summary salary ON c.id = salary.customer_id
LEFT JOIN insights_insurance_summary ins ON c.id = ins.customer_id
LEFT JOIN insights_transactions_summary txn ON c.id = txn.customer_id
LEFT JOIN insights_loan_summary loan ON c.id = loan.customer_id
ORDER BY c.id;
