                ...
                if message_type == "SALARY_CREDIT":
                    ...
                    if not employer or employer == "General Transaction":
                        await conn.execute("""
                            ...
                            FROM customers WHERE phone_number = $7
                            ON CONFLICT ON CONSTRAINT unique_general_tx DO NOTHING
                        """, message_type, data.get('bank_name'), data.get('account_number'),
                             data.get('amount'), data.get('transaction_date'),
                             data.get('available_balance'), phone_number)
                    ...
                    await conn.execute("""
                        ...
                        FROM customers WHERE phone_number = $9
                    ...
                    """, data.get('bank_name'), data.get('account_number'), data.get('amount'),
                         ...
                         data.get('total_outstanding'), phone_number)
                    ...
                    """, data.get('policy_number'), data.get('insurance_company'),
                         data.get('insurance_type'), data.get('amount'),
                         data.get('transaction_date'), phone_number)
...
# UPDATED: process_single_message() to accept customer_info and pass it to store function
async def process_single_message(date_str: str, message: str, customer_info: Dict[str, Any]):
    ...
    # Debug: log received customer info at message processing start
    logger.info(f"Processing message for customer: {customer_info}")
